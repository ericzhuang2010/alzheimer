# Phase 18 `call_key_driver_significant_returns.tsv`: Column Guide
...
even though every value is physically stored on a significant gene × run row.
Values defined per gene × broad network are repeated on every significant run
row representing that candidate unit.
...
| 13 | `returned_by_call_key_drivers` | Per gene × run; significance assessed within the run |
| 14 | `published_best_layer` | Per gene × run |
| 15 | `published_overlap_count` | Per gene × run |
| 16 | `published_neighborhood_size` | Per gene × run |
| 17 | `published_non_neighborhood_size` | Per gene × run |
| 18 | `published_signature_size` | Per gene × run |
| 19 | `published_fold_enrichment` | Per gene × run |
| 20 | `published_log_p_value` | Per gene × run |
| 21 | `published_raw_p_value` | Per gene × run |
| 22 | `published_adjusted_p_value` | Per gene × run; BH corrected within the run |
| 23 | `published_is_signature` | Per gene × run |
| 24 | `published_is_root_node` | Per gene × run/network topology |
| 25 | `published_global_key_driver` | Per gene × run; redundancy assessed within the run |
| 26 | `published_overlap_items` | Per gene × run |
| 27 | `case_order` | Per gene |
| 28 | `case_id` | Per gene |
| 29 | `case_label` | Per gene |
| 30 | `is_core_mito` | Per gene |
| 31 | `mitocarta_canonical_symbol` | Per gene |
| 32 | `query_member` | Per gene × run |
| 33 | `test_status` | Per gene × run |
| 34 | `usable_test` | Per gene × run |
| 35 | `explicit_family_member` | Per gene × run |
| 36 | `original_layer` | Per gene × run |
| 37 | `original_overlap_count` | Per gene × run |
| 38 | `original_neighborhood_size` | Per gene × run |
| 39 | `original_non_neighborhood_size` | Per gene × run |
| 40 | `original_signature_size` | Per gene × run |
| 41 | `original_fold_enrichment` | Per gene × run |
| 42 | `original_log_p` | Per gene × run |
| 43 | `original_raw_p` | Per gene × run |
| 44 | `original_run_q` | Per gene × run; BH corrected within the run |
| 45 | `self_excluded` | Per gene × run |
| 46 | `final_layer` | Per gene × run |
| 47 | `final_overlap_count` | Per gene × run |
| 48 | `final_neighborhood_size` | Per gene × run |
| 49 | `final_non_neighborhood_size` | Per gene × run |
| 50 | `final_signature_size` | Per gene × run |
| 51 | `final_background_size` | Per gene × run |
| 52 | `final_fold_enrichment` | Per gene × run |
| 53 | `final_log_p` | Per gene × run |
| 54 | `final_raw_p` | Per gene × run |
| 55 | `final_run_q` | Per gene × run; BH corrected within the run |
| 56 | `other_query_overlap` | Per gene × run |
| 57 | `support_overlap_pass` | Per gene × run |
| 58 | `support_fold_pass` | Per gene × run |
| 59 | `support_run_q_pass` | Per gene × run |
| 60 | `conservative_support` | Per gene × run |
| 61 | `mito_tier` | Per gene |
| 62 | `genome_origin` | Per gene |
| 63 | `is_mtdna_gene` | Per gene |
| 64 | `extended_reference_member` | Per gene |
| 65 | `mapping_status` | Per gene |
| 66 | `phase03_mitocarta_match_type` | Per gene |
| 67 | `eligible_run_count` | Per broad network; fixed for every gene in that network |
| 68 | `usable_run_count` | Per gene × broad network |
| 69 | `explicit_run_count` | Per gene × broad network |
| 70 | `implicit_run_count` | Per gene × broad network |
| 71 | `missing_run_count` | Per gene × broad network |
| 72 | `coverage_numerator` | Per gene × broad network |
| 73 | `coverage_denominator` | Per broad network; repeated per gene |
| 74 | `coverage_fraction` | Per gene × broad network |
| 75 | `coverage_pass` | Per gene × broad network |
| 76 | `conservative_support_count` | Per gene × broad network |
| 77 | `conservative_support_pass` | Per gene × broad network |
| 78 | `recurrence_fraction` | Per gene × broad network |
| 79 | `supporting_fine_cell_type_count` | Per gene × broad network |
| 80 | `supporting_fine_cell_types` | Per gene × broad network |
| 81 | `supporting_group_count` | Per gene × broad network |
| 82 | `supporting_groups` | Per gene × broad network |
| 83 | `supporting_direction_count` | Per gene × broad network |
| 84 | `supporting_directions` | Per gene × broad network |
| 85 | `median_support_fold_enrichment` | Per gene × broad network |
| 86 | `maximum_support_fold_enrichment` | Per gene × broad network |
| 87 | `aggregate_acat_p` | Per gene × broad network |
| 88 | `aggregate_acat_q` | Per gene × broad network; BH corrected within the broad network across both driver classes |
| 89 | `aggregate_q_pass` | Per gene × broad network |
| 90 | `missing_as_one_acat_p` | Per gene × broad network |
| 91 | `missing_as_one_acat_q` | Per gene × broad network; BH corrected within the broad network |
| 92 | `mean_log_p_score` | Per gene × broad network |
| 93 | `terminal_candidate_status` | Per gene × broad network |
| 94 | `within_case_rank` | Per gene × broad network; ranked within broad network × driver class |
| 95 | `top5_display` | Per gene × broad network |
| 96 | `stability_assessable_repetitions` | Per gene × broad network |
| 97 | `stability_nominal_fraction` | Per gene × broad network |
| 98 | `stability_q_fraction` | Per gene × broad network |
| 99 | `stability_candidate_fraction` | Per gene × broad network |
| 100 | `stability_worst_rank` | Per gene × broad network; ranks evaluated within broad network × driver class |
| 101 | `evidence_tier` | Per gene × broad network |
| 102 | `case_driver_candidate_count` | Per broad network × driver class |
| 103 | `case_displayed_candidate_count` | Per broad network × driver class |
