...
`results/local_pilot/11_pathway_similarity/` with status `nonfinal_smoke_test`, 2
...
| `.gitignore` | Add an explicit `11_pathway_similarity` result exception only if the project intends to track generated tables. Respect MSigDB redistribution terms separately. |
...
```text
results/local_pilot/11_pathway_similarity/
```
...
Rscript -e '
library(data.table)
root <- "results/local_pilot/11_pathway_similarity"
...
Rscript -e '
library(data.table)
root <- "results/minerva_production/11_pathway_similarity"
status <- fread(file.path(root, "pathway_status.tsv"))
checks <- fread(file.path(root, "pathway_checks.tsv"))
...
[Phase 11 Figures 3–6 generation guide](../figures/phase_11_mitochondrial_figures_3_to_6_guide.md)
and write outside `11_pathway_similarity/`.
...
- [x] Confirm no figure artifact exists under local `11_pathway_similarity/`.
