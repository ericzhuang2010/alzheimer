# Astrocytes RDS: Concise Structure Summary

This document summarizes `Astrocytes.rds` from the completed Minerva inspection in `results/rds_structure_summaries.json`. It describes structure and dimensions; it does not contain the expression matrices.
...
| Seurat object version | 3.1.1 |
| File size on disk | 1.6 GiB |
| Approximate size after loading in R | 8.2 GiB |
| Genes/features | 33,538 |
| Nuclei (called cells by Seurat) | 149,558 |
| Donors (`projid`) | 427 |
| Fine cell types | 3 |
| Observed donor × fine-cell-type combinations | 1,235 |
| Possible donor × fine-cell-type combinations | 1,281 |
| Donor × fine-cell-type coverage | 96.4% |
| Active assay | `RNA` |
...
|---|---:|---:|---:|
| `RNA@counts` | 33,538 × 149,558 | 427 donors; 3 fine types | Raw UMI counts; 354,618,113 nonzero entries; 827,101,045 total UMIs |
| `RNA@data` | 33,538 × 149,558 | 427 donors; 3 fine types | Normalized expression; 354,618,113 nonzero entries |
...
| `meta.data` | 149,558 × 2 | 427 donors; 3 fine types | Per-nucleus donor and fine-cell-type assignments |
| `active.ident` | Length 149,558 | Cell-level | Matches fine-cell-type metadata |
| `reductions$umap` | 149,558 × 2 | Cell-level | Saved `umap` cell embedding; assay used: `integrated`, but that assay is not stored |

## Fine-cell-type composition

| Fine cell type | Nuclei | Donors represented | Missing donor IDs |
|---|---:|---:|---:|
| `Ast CHI3L1` | 24,252 | 387 | 0 |
| `Ast DPP10` | 32,779 | 424 | 0 |
| `Ast GRM3` | 92,527 | 424 | 0 |
| **Total** | **149,558** | **427 unique** | **0** |
...
|---|---:|---:|
| `reductions$umap` | 149,558 × 2 | Saved `umap` cell embedding; assay used: `integrated`, but that assay is not stored |

| Tool component | Dimensions | Interpretation |
|---|---:|---:|
| `tools$Integration@anchors` | 2,493,390 × 5 | Internal Seurat integration anchor table; not donor/sample metadata |
| `tools$TransferData$weights.matrix` | 4,094 × 149,558 | Sparse transfer weights; 7,477,900 nonzero entries |
...
`Astrocytes.rds` contains raw and normalized RNA expression for 33,538 genes across 149,558 nuclei from 427 donors and 3 fine cell type(s). Donor-aware analyses should use `projid` as the biological replicate and `cell_type_high_resolution` as the cell-type label.
