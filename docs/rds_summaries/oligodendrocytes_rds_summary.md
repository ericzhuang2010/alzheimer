# Oligodendrocytes RDS: Concise Structure Summary

This document summarizes `Oligodendrocytes.rds` from the completed Minerva inspection in `results/rds_structure_summaries.json`. It describes structure and dimensions; it does not contain the expression matrices.
...
| File size on disk | 5.0 GiB |
| Approximate size after loading in R | 27.2 GiB |
| Genes/features | 33,538 |
| Nuclei (called cells by Seurat) | 645,142 |
| Donors (`projid`) | 427 |
| Fine cell types | 1 |
...
|---|---:|---:|---:|
| `RNA@counts` | 33,538 × 645,142 | 427 donors; 1 fine types | Raw UMI counts; 1,203,844,320 nonzero entries; 2,576,924,441 total UMIs |
| `RNA@data` | 33,538 × 645,142 | 427 donors; 1 fine types | Normalized expression; 1,203,844,320 nonzero entries |
...
| `meta.data` | 645,142 × 2 | 427 donors; 1 fine types | Per-nucleus donor and fine-cell-type assignments |
| `active.ident` | Length 645,142 | Cell-level | Matches fine-cell-type metadata |
...
|---|---:|---:|---:|---:|---:|
| 10 | 647 | 1,291 | 1,510.9 | 2,070.5 | 9,535 |
...
|---|---:|---:|
| `tools$Integration@anchors` | 666,098 × 5 | Internal Seurat integration anchor table; not donor/sample metadata |
...
`Oligodendrocytes.rds` contains raw and normalized RNA expression for 33,538 genes across 645,142 nuclei from 427 donors and 1 fine cell type(s). Donor-aware analyses should use `projid` as the biological replicate and `cell_type_high_resolution` as the cell-type label.
