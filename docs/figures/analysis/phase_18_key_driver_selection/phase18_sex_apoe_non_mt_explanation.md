# Phase 18 non-MT Driver Sex/APOE Figure

This figure shows where the non-MT key-driver evidence occurs across sex,
APOE group, AD direction, and broad cell-type network. It is a validated
descriptive figure—all validation checks passed—but it does not test sex-by-APOE
interactions.
...
- **Circle**: solid dot means the context appeared in the non-MT circle; an
...
The current `call_key_driver_returns.tsv` supplies selection and network-level
statistics. Its rows contain explicit tests only, so the validated archived
candidate-test table is used to distinguish implicit zero-overlap P = 1 tests
from genuinely unavailable tests when reconstructing these strata.
...
5. **Rank**  
   The candidate's rank among non-MT candidates within that broad network.
...
[sex_apoe_non_mt](../../../../results/figures/analysis/phase_18_key_driver_selection/sex_apoe_non_mt/).
