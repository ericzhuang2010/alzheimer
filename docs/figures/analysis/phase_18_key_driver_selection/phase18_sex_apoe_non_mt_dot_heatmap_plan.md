# Phase 18 non-MT Driver Sex/APOE Dot-Heatmap Plan

> Current selection, ranks, and network summaries come from
> `call_key_driver_returns.tsv`. The archived candidate-test table is retained
> only to distinguish implicit P = 1 tests from genuinely unavailable tests
> when reconstructing the 12 sex/APOE strata.
...
genes displayed in the non-MT circular graph. Its visual grammar is based on:
...
> Sex- and APOE-stratified support for non-MT key-driver candidates

Working subtitle:

> Primary AD-up and AD-down mitochondrial queries; descriptive strata, not interaction tests

The figure asks where the non-MT drivers receive their run-level evidence:
...
- one row per passing non-MT gene-network context involving a circle gene;
...
1. Derive the 15 unique genes in the official non-MT circle.
2. Reconcile the 21 displayed gene-network contexts.
3. For those genes, include all passing non-MT gene-network contexts.
...
eligible_query_count
  = number of included fine-cell-type runs in the network/group/direction cell
    for which the non-MT gene is eligible
...
For non-MT, membership is not query-dependent, but eligibility and usability
...
| `key_driver_case_manifest.tsv` | Verify the exact non-MT definition. |
...
scripts/figures/analysis/phase_18_key_driver_selection/
  ...
  plot_phase18_sex_apoe_non_mt.py
  visualize_phase18_sex_apoe_non_mt.R
```
...
1. **Preflight:** verify validated status, checks, source artifacts, manifests,
   non-MT definition, and primary run scope.
...
`phase18_sex_apoe_non_mt_plot_data.tsv` contains exactly one row per heatmap
cell and at least:
...
`phase18_sex_apoe_non_mt_row_annotations.tsv` contains exactly one row per
...
```text
phase18_sex_apoe_non_mt_aggregation_audit.tsv.gz
```
...
```text
results/figures/analysis/phase_18_key_driver_selection/sex_apoe_non_mt/
```

Generated files:

```text
phase18_sex_apoe_non_mt.svg
phase18_sex_apoe_non_mt.pdf
phase18_sex_apoe_non_mt.png
phase18_sex_apoe_non_mt_plot_data.tsv
phase18_sex_apoe_non_mt_row_annotations.tsv
phase18_sex_apoe_non_mt_aggregation_audit.tsv.gz
phase18_sex_apoe_non_mt_caption.md
phase18_sex_apoe_non_mt_methods.md
phase18_sex_apoe_non_mt_manifest.tsv
phase18_sex_apoe_non_mt_checks.tsv
phase18_sex_apoe_non_mt_artifacts.tsv
phase18_sex_apoe_non_mt_status.tsv
```
...
- non-MT means outside core MitoCarta, not necessarily unrelated to
