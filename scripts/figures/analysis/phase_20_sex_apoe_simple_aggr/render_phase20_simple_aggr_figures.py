#!/usr/bin/env python3
"""Render non-MT figures from the existing Phase 20 simple aggregation.

This renderer does not rerun KDA or ACAT. It filters the validated all-class
category aggregate to non-MT rows, assigns non-MT-only display ranks within
each sex/APOE-by-broad-cell category, and writes two publication-ready figure
bundles: driver recurrence and top-five candidates.
"""
...
os.environ.setdefault(
    "MPLCONFIGDIR",
    str(Path(tempfile.gettempdir()) / "phase20_simple_aggr_matplotlib"),
)
...
SOURCE_ANALYSIS_ID = "phase20_simple_returned_only_acat_v1"
FIGURE_SCHEMA_ROOT = "phase20_simple_aggr_non_mt_figure_v1"
    ...
    plt.rcParams.update(
        {
            ...
            "pdf.fonttype": 42,
            "svg.fonttype": "none",
            "svg.hashsalt": "phase20_simple_aggr_non_mt_v1",
...
def validate_source(result_dir: Path) -> dict[str, Any]:
    ...
    if int(status_row["included_run_count"]) != 295:
        raise ValueError("Figure renderer requires the frozen 295-call source")
...
def derive_non_mt_rows(source: dict[str, Any]) -> pd.DataFrame:
    ...
    non_mt["source_all_gene_rank"] = pd.to_numeric(
...
def plot_recurrence(recurrence: pd.DataFrame) -> plt.Figure:
    ...
    axis.set_xlim(0, float(ordered["category_count"].max()) + 1.35)
    axis.set_xlabel("Number of non-MT key-driver categories")
    axis.set_ylabel("")
    add_title(
        axis,
        "Most recurrent simple-aggregation non-MT key drivers",
...
def draw_top5_panel(axis: plt.Axes, rows: pd.DataFrame, sex: str, panel: str) -> None:
...
def plot_top5(top5: pd.DataFrame) -> plt.Figure:
    ...
    figure.text(
        0.06,
        0.94,
        "Re-ranked after excluding MT genes; categories without a non-MT return are omitted",
        ha="left",
        color=MUTED_COLOR,
        fontsize=9,
    )
    ...
    figure.subplots_adjust(left=0.15, right=0.985, top=0.87, bottom=0.11, wspace=0.35)
    return figure
...
def common_checks(source: dict[str, Any], non_mt: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    return [
        ...
        check("all_class_category_rows", len(source["categories"]), 1298, len(source["categories"]) == 1298),
        check("non_mt_category_rows", len(non_mt), 689, len(non_mt) == 689),
        check("non_mt_unique_genes", non_mt["current_symbol"].nunique(), 433, non_mt["current_symbol"].nunique() == 433),
        check("categories_with_non_mt", len(category_counts), 32, len(category_counts) == 32),
        ...
        check("category_gene_keys_unique", len(set(keys)), 689, len(set(keys)) == len(non_mt)),
        check("returned_call_occurrences", int(non_mt["returned_call_count"].sum()), 1033, int(non_mt["returned_call_count"].sum()) == 1033),
        check("singleton_category_units", int(non_mt["returned_call_count"].eq(1).sum()), 538, int(non_mt["returned_call_count"].eq(1).sum()) == 538),
        check("recurrent_category_units", int(non_mt["returned_call_count"].ge(2).sum()), 151, int(non_mt["returned_call_count"].ge(2).sum()) == 151),
...
def recurrence_checks(
    ...
    return common_checks(source, non_mt) + [
        check("recurrence_plot_rows", len(recurrence), 20, len(recurrence) == 20),
        check("recurrence_genes_unique", recurrence["current_symbol"].nunique(), 20, recurrence["current_symbol"].nunique() == 20),
        check("recurrence_source_parity", "TRUE" if parity else "FALSE", "TRUE", parity),
        check("maximum_category_recurrence", int(recurrence["category_count"].max()), 12, int(recurrence["category_count"].max()) == 12),
        check("top_recurrent_gene", recurrence.iloc[0]["current_symbol"], "RPS15", recurrence.iloc[0]["current_symbol"] == "RPS15"),
    ]
...
def top5_checks(
    ...
    return common_checks(source, non_mt) + [
        check("top5_plot_rows", len(top5), 149, len(top5) == 149),
        check("top5_category_count", len(per_category), 32, len(per_category) == 32),
...
def save_bundle(
    *,
    ...
    stem = f"phase20_simple_aggr_{figure_id}"
    paths = {
...
def main() -> int:
    ...
    figure_root = (
        ...
        if args.figure_root
        else root / "results" / "figures" / "analysis" / "phase_20_sex_apoe_simple_aggr"
    )
...
The score is the requested exploratory returned-only value: a singleton stock within-call BH q is passed through unchanged, whereas two or more returned q values are combined by equal-weight ACAT. It is post-selected and is not a formally FDR-controlled cross-call q value; the figures are descriptive rankings of stock-significant returns."""
    ...
    recurrence_caption = """# Phase 20 simple-aggregation driver recurrence

The 20 most recurrent non-MT key drivers across sex/APOE-by-broad-cell categories. Bar length is the number of categories containing the gene. Fill records how many of those category occurrences combine two or more significant call returns by ACAT; the remainder are one-call q passthroughs."""
...
For recurrence, each gene is counted at most once in each category. Genes are ordered by category count (descending), best returned-q ACAT score (ascending), and symbol; the first 20 are displayed."""
    ...
    top5_caption = """# Phase 20 simple-aggregation top-five candidates

Up to five non-MT key drivers per sex/APOE-by-broad-cell category, split into female and male panels. Blue tiles represent scores ACAT-combined across at least two significant call returns; orange tiles represent a single within-call q passthrough. Categories without any non-MT return are omitted."""
...
For the top-five display, ranks 1–5 after non-MT filtering are retained without backfilling or an additional significance threshold. This leaves 149 plotted gene-category rows across 32 categories. The word “candidates” names the requested display and does not imply a new confirmatory error-rate claim."""
    ...
    manifest_path = figure_root / "phase20_simple_aggr_figure_manifest.tsv"
