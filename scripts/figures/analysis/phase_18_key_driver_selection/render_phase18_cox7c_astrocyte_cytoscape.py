#!/usr/bin/env python3
"""Render the Phase 18 COX7C astrocyte consensus network with Cytoscape."""
...
OUTPUT_STEM = "phase18_cox7c_astrocyte_consensus_network_pathways"
ANALYSIS_PREFIX = "phase18_cox7c_astrocyte_consensus"
NETWORK_TITLE = "Phase 18 COX7C radial astrocyte consensus network"
COLLECTION_TITLE = "Phase 18 COX7C deep dive"
STYLE_NAME = "Phase18 COX7C astrocyte radial consensus pathway outlines"

SUPPORTING_RUNS = 2
QUERY_HIT_THRESHOLD = 1
EXPECTED_NODE_COUNT = 23
EXPECTED_EDGE_COUNT = 23
EXPECTED_DIRECT_INCOMING = 1
EXPECTED_DIRECT_OUTGOING = 15
EXPECTED_QUERY_HITS = 10
...
PATHWAY_THEMES = {
    "O": ("ETC / oxidative phosphorylation", "#0072B2"),
    "C": ("Cristae formation", "#009E73"),
    "R": ("Cytosolic ribosome", "#6F4E9C"),
}
...
def node_display_size(gene: str, occurrence_count: int) -> float:
    if gene == "COX7C":
        return 156.0
    recurrence_size = 112.0 + 12.0 * occurrence_count
    label_size = 108.0 + 8.0 * max(0, len(gene) - 5)
...
def load_inputs(input_dir: Path) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    require(len(nodes) == EXPECTED_NODE_COUNT, f"Expected 23 nodes, found {len(nodes)}")
    require(len(edges) == EXPECTED_EDGE_COUNT, f"Expected 23 edges, found {len(edges)}")
    ...
    nodes["directed_depth_from_cox7c"] = pd.to_numeric(nodes["directed_depth_from_cox7c"], errors="coerce")
    nodes["upstream_depth_to_cox7c"] = pd.to_numeric(nodes["upstream_depth_to_cox7c"], errors="coerce")
    edges["direct_edge_bool"] = as_bool(edges["is_directly_incident_to_cox7c"])
    ...
    require(int(as_bool(nodes["is_direct_incoming_neighbor"]).sum()) == EXPECTED_DIRECT_INCOMING, "Direct-parent count changed")
    require(int(as_bool(nodes["is_direct_outgoing_neighbor"]).sum()) == EXPECTED_DIRECT_OUTGOING, "Direct-child count changed")
...
def radial_positions(nodes: pd.DataFrame, edges: pd.DataFrame) -> tuple[dict[str, tuple[float, float]], float, str]:
    ...
    require(nx.is_directed_acyclic_graph(graph), "Displayed COX7C graph is not acyclic")
    require(nx.is_weakly_connected(graph), "Displayed COX7C graph is disconnected")
    require(set(graph.predecessors("COX7C")) == {"RPL11"}, "COX7C direct parent changed")
    require(set(graph.successors("COX7C")) == set(D1_ANGLES), "COX7C direct children changed")
    require(set(nx.ancestors(graph, "COX7C")) == {"RPL11", "RPLP1"}, "Upstream chain changed")
    require(graph.has_edge("RPLP1", "RPL27"), "RPLP1-to-RPL27 cross-context edge is absent")
...
def prepare_cytoscape_tables(
    ...
    def display_label(row: pd.Series) -> str:
        if pd.notna(row["upstream_depth_to_cox7c"]):
            return f"{row['gene']}\nU{int(row['upstream_depth_to_cox7c'])}"
...
def add_annotations(network_suid: int) -> None:
    ...
    title_x, title_y = -920.0, -1080.0
    add_text("COX7C-centered astrocyte consensus network", title_x, title_y, "figure_title", 29, "bold", "#111111")
    add_text(
    ...
    legend_x, legend_top = 1080.0, -760.0
    text_x = legend_x + 46.0
    add_text("HOW TO READ", legend_x, legend_top, "legend_title", 20, "bold", "#111111")
    add_text("Node size and x/2 show neighborhood recurrence", legend_x, legend_top + 52, "legend_recurrence_1")
    add_text("across the 2 supporting runs", legend_x, legend_top + 86, "legend_recurrence_2")
    add_text("U1/U2: upstream distance to COX7C", legend_x, legend_top + 126, "legend_upstream")
    add_text("Thick black border: mitochondrial query hit at >=1/2", legend_x, legend_top + 166, "legend_query_hit")
    ...
    add_text("Nonredundant representatives; all have BH FDR < 0.05", legend_x, pathway_heading_y + 39, "legend_pathway_note", 15, color="#555555")
    ...
    add_text("U1-U2: upstream context; D1-D2: minimum downstream steps", legend_x, radial_heading_y + 43, "legend_radial_text")

    scope_heading_y = radial_heading_y + 110
    add_text("DISPLAY SCOPE", legend_x, scope_heading_y, "legend_scope_heading", 17, "bold", "#111111")
    add_text("23 nodes / 23 edges", legend_x, scope_heading_y + 43, "legend_scope_counts")
    add_text("All COX7C edges: 1 incoming, 15 outgoing", legend_x, scope_heading_y + 80, "legend_scope_direct")
    add_text("D2 paths to all 10 observed self-excluded query hits", legend_x, scope_heading_y + 117, "legend_scope_hits")
    add_text("All model edges among displayed nodes are retained", legend_x, scope_heading_y + 154, "legend_scope_edges")
    add_text(
        "Display threshold >=1/2 is a coverage choice, not a significance cutoff",
        legend_x,
        scope_heading_y + 202,
...
def render(input_dir: Path, output_dir: Path, png_zoom: int) -> None:
    ...
    p4c.set_node_color_bypass(["COX7C"], "#111111", network=network_suid)
    p4c.set_node_label_color_bypass(["COX7C"], "#FFFFFF", network=network_suid)
    p4c.set_node_border_color_bypass(["COX7C"], "#111111", network=network_suid)
    p4c.set_node_border_width_bypass(["COX7C"], 5.0, network=network_suid)
    p4c.set_node_font_size_bypass(["COX7C"], 25, network=network_suid)
    p4c.set_node_property_bypass(["COX7C"], [156.0], "NODE_SIZE", network=network_suid)
    ...
    export_log: dict[str, object] = {
        ...
        "threshold_rationale": "1/2 retains all 10 self-excluded hits in a readable 23-node graph; 2/2 retains only 4 hits and removes 4 nodes",
