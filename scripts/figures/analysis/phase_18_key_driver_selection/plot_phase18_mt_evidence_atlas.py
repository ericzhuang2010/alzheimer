#!/usr/bin/env python3
"""Build the current Phase 18 MT-driver evidence atlas.
...
MPL_CACHE = Path(tempfile.gettempdir()) / "phase18_mt_atlas_mplconfig"
MPL_CACHE.mkdir(parents=True, exist_ok=True)
os.environ.setdefault("MPLCONFIGDIR", str(MPL_CACHE))
FONT_CACHE = Path(tempfile.gettempdir()) / "phase18_mt_atlas_fontcache"
...
SCHEMA = "phase18_mt_evidence_atlas_v2"
INPUT_SCHEMA = "phase18_call_key_driver_returns_v1"
CASE_ID = "mt_driver"
FIGURE_ID = "phase18_evidence_atlas_mt"
DEFAULT_INPUT = (
...
EXPECTED_SELECTED_GENES = 10
EXPECTED_DISPLAYED_CONTEXTS = 26
EXPECTED_PASSING_CONTEXTS = 30
EXPECTED_SELECTED_TEST_ROWS = 836
EXPECTED_SELECTED_SIGNIFICANT_ROWS = 396
EXPECTED_SELECTED_SUPPORT_ROWS = 278
...
def load_bundle(input_path: Path, evidence_cap: float) -> dict[str, Any]:
    ...
    require(
        len(displayed_keys) == EXPECTED_DISPLAYED_CONTEXTS,
        f"Expected {EXPECTED_DISPLAYED_CONTEXTS} displayed MT contexts",
    )
    require(
        len(circle_genes) == EXPECTED_SELECTED_GENES,
        f"Expected {EXPECTED_SELECTED_GENES} selected MT genes",
    )
    require(circle_genes == EXPECTED_GENES, "Selected MT gene set changed")
    require(
        len(passing_keys) == EXPECTED_PASSING_CONTEXTS,
        f"Expected {EXPECTED_PASSING_CONTEXTS} passing contexts for selected genes",
    )
    require(
        passing_keys - displayed_keys == EXPECTED_EXTRA_CONTEXTS,
        "Unexpected passing contexts below display cap",
    )
    ...
    for gene in circle_genes:
        ...
        require(all(truth(row["is_core_mito"]) for row in rows), f"MT driver is not core MitoCarta: {gene}")
        summary_rows.append(
            {
...
def render_panel_a(ax: plt.Axes, bundle: Mapping[str, Any]) -> None:
    ...
    footer_y = row_count + 0.05
    cmap = matplotlib.colormaps["cividis"]
    ax.set_xlim(-1.30, 7.50)
    ax.set_ylim(row_count + 0.90, -2.20)
    ax.axis("off")
    panel_title(
        ax,
        "A",
        "Network evidence matrix",
        "Tile number = within-network MT-driver rank",
    )
    ...
    ax.add_patch(mpatches.Rectangle((gradient_start, footer_y), gradient_end - gradient_start, 0.22, fill=False, edgecolor="#555555", linewidth=0.7))
    ax.text((gradient_start + gradient_end) / 2, footer_y - 0.20, "capped −log10(ACAT q)", fontsize=6.8, fontweight="bold", ha="center")
    ax.text(gradient_start, footer_y + 0.38, "0", fontsize=6.5, ha="left")
    ax.text(gradient_end, footer_y + 0.38, f"{evidence_cap:g}", fontsize=6.5, ha="right")

    ax.add_patch(mpatches.Rectangle((3.0, footer_y - 0.03), 0.30, 0.28, facecolor="white", edgecolor="#202020", linewidth=1.2))
    ax.text(3.38, footer_y + 0.11, "circle top five", fontsize=6.6, va="center")
    ax.add_patch(mpatches.Rectangle((4.72, footer_y - 0.03), 0.30, 0.28, facecolor="white", edgecolor="#666666", linewidth=0.8, linestyle="--"))
    ax.text(5.10, footer_y + 0.11, "passing, below cap", fontsize=6.6, va="center")
    ax.scatter([3.0], [footer_y + 0.62], marker="D", s=15, color="#111111")
    ax.text(3.18, footer_y + 0.62, "broader mitochondrial reference", fontsize=6.6, va="center")
...
def render_panel_c(ax: plt.Axes, bundle: Mapping[str, Any]) -> None:
    ...
    row_count = len(bundle["summary_rows"])
    ...
    for x in (retention_start, retention_end, rank_start, rank_end):
        ax.plot([x, x], [-0.05, row_count - 0.45], color="#B8BEC3", lw=0.7)
    ...
    ax.plot([retention_reference, retention_reference], [-0.05, row_count - 0.45], color="#92999F", lw=0.8, linestyle=":")
    ax.plot([rank_reference, rank_reference], [-0.05, row_count - 0.45], color="#92999F", lw=0.8, linestyle=":")
    ...
    for value in (0, 0.5, 1):
        ...
        ax.text(x, row_count + 0.05, f"{value:g}", fontsize=6.3, ha="center")
    for value in (1, 5, 10, 15, 20, 25):
        x = rank_start + (value - 1) / (25 - 1) * (rank_end - rank_start)
        ax.text(x, row_count + 0.05, str(value), fontsize=6.1, ha="center")
    ax.text(0.02, row_count + 0.43, "Colors match Panel A.\n× = stability not assessable.", fontsize=6.1, color=MID, ha="left", linespacing=1.15)


def render_figure(bundle: Mapping[str, Any], width: float, height: float) -> plt.Figure:
    configure_style()
    fig = plt.figure(figsize=(width, height), facecolor="white")
    fig.text(0.018, 0.985, "Evidence atlas for selected MT key drivers", fontsize=14, fontweight="bold", ha="left", va="top", color=TEXT)
    fig.text(0.018, 0.947, "Drivers inside the fixed 1,136-gene core MitoCarta inventory", fontsize=9.5, color=MID, ha="left", va="top")
    fig.text(
        0.018,
        0.918,
        f"{len(bundle['summary_rows'])} selected genes  •  "
        f"{bundle['displayed_contexts']} circle-displayed contexts  •  "
        f"{bundle['passing_contexts']} total passing contexts  •  "
        f"{bundle['run_count']} included KDA calls",
        fontsize=7.5,
        color=MID,
        ha="left",
        va="top",
    ...
    for extension in ("svg", "pdf", "png"):
        ...
        if extension == "pdf":
            metadata: dict[str, Any] = {
                "Creator": "Phase 18 MT evidence-atlas renderer",
                "CreationDate": None,
                "ModDate": None,
            }
        elif extension == "svg":
            ...
            metadata = {"Software": "Phase 18 MT evidence-atlas renderer"}
        fig.savefig(
            temporary,
            format=extension,
...
CAPTION = """# Phase 18 MT-driver evidence-atlas caption
...
The atlas was generated directly from `call_key_driver_returns.tsv` with schema `{INPUT_SCHEMA}`. The renderer required 95,557 explicit gene × run rows, 161 included KDA calls, one unique `kda_run_id + key_driver` row, and constant aggregate fields within each `broad_network + key_driver + case_id` unit. The selected-gene universe was the 10 unique `mt_driver` symbols with `top5_display = TRUE`; these yielded 26 displayed contexts and 30 passing contexts after retaining every `terminal_candidate_status = driver_candidate` row for those genes.
...
def build_checks(
    ...
    checks = [
        ...
        check_record("selected_genes", len(summary) == EXPECTED_SELECTED_GENES, len(summary), EXPECTED_SELECTED_GENES),
        check_record("displayed_contexts", bundle["displayed_contexts"] == EXPECTED_DISPLAYED_CONTEXTS, bundle["displayed_contexts"], EXPECTED_DISPLAYED_CONTEXTS),
        check_record("passing_contexts", bundle["passing_contexts"] == EXPECTED_PASSING_CONTEXTS, bundle["passing_contexts"], EXPECTED_PASSING_CONTEXTS),
        check_record("panel_a_grid", len(plots) == EXPECTED_SELECTED_GENES * len(NETWORK_ORDER), len(plots), EXPECTED_SELECTED_GENES * len(NETWORK_ORDER)),
        check_record("panel_a_keys", len({(row["current_symbol"], row["broad_network"]) for row in plots}) == EXPECTED_SELECTED_GENES * len(NETWORK_ORDER), len({(row["current_symbol"], row["broad_network"]) for row in plots}), EXPECTED_SELECTED_GENES * len(NETWORK_ORDER)),
...
def manifest_rows(
    ...
    fields = [
        ...
        ("selected_gene_count", EXPECTED_SELECTED_GENES),
        ("circle_display_context_count", EXPECTED_DISPLAYED_CONTEXTS),
        ("passing_context_count", EXPECTED_PASSING_CONTEXTS),
...
def validate_output(directory: Path, expected_visual_status: str | None = None) -> None:
    ...
    print(f"Validated Phase 18 MT evidence atlas: {directory}")
...
def publish(
    ...
    staging = Path(tempfile.mkdtemp(prefix=".phase18_mt_atlas.staging.", dir=output_dir.parent))
    try:
        ...
        print(
            "Panel B totals: "
            f"{bundle['selected_test_rows']} tested gene-run rows; "
            f"{bundle['selected_significant_rows']} significant KDA returns; "
            f"{bundle['selected_support_rows']} conservative-support runs"
        )
    except Exception:
        shutil.rmtree(staging, ignore_errors=True)
        raise
