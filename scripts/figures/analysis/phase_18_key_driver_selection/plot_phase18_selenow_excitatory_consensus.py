#!/usr/bin/env python3
"""Build and render the Phase 18 SELENOW excitatory consensus network in Cytoscape."""
...
NETWORK = ROOT / "data/bayesian_network/Excitatory_neurons/result.links3.links.txt"
ANNOTATION = ROOT / "results/minerva_production/09_annotate_genes/gene_annotation_master.tsv.gz"
MAST_FILES = [
    ROOT / "results/minerva_production/08_deg_fine/excitatory_set1.yu_mast_de.tsv.gz",
    ROOT / "results/minerva_production/08_deg_fine/excitatory_set2.yu_mast_de.tsv.gz",
    ROOT / "results/minerva_production/08_deg_fine/excitatory_set3.yu_mast_de.tsv.gz",
]
MSIGDB = ROOT / "data/reference/msigdb/c2.cp.v2026.1.Hs.symbols.gmt"

DRIVER = "SELENOW"
NETWORK_NAME = "Excitatory_neurons"
NETWORK_LABEL = "Excitatory neurons"
SUPPORTING_RUNS = 14
QUERY_HIT_THRESHOLD = 4
MAX_DOWNSTREAM_DEPTH = 3
MAX_UPSTREAM_DEPTH = 2
SCHEMA = "phase18_selenow_excitatory_consensus_v1"
OUTPUT_PREFIX = "phase18_selenow_excitatory_consensus"
FIGURE_STEM = "phase18_selenow_excitatory_consensus_network_pathways"
STYLE_NAME = "Phase18 SELENOW excitatory radial consensus pathway outlines"

EXPECTED_DIRECT_IN = {"ATP5MPL"}
EXPECTED_DIRECT_OUT = {"BRK1", "COA3", "DNAJB9", "LAGE3", "PRELID1", "PSEN1", "STRAP"}
EXPECTED_UPSTREAM_CONTEXT = {"ATP5MPL", "LAMTOR5"}
EXPECTED_QUERY_HITS = {
    "CCDC127", "CHCHD10", "CLPP", "COA3", "CYC1", "DMAC2", "GUK1",
    "MRPL34", "MRPL4", "MRPS34", "NDUFB11", "PRELID1", "SLC25A4",
}
PATHWAY_THEMES = (
    {
        "pathway": "REACTOME_AEROBIC_RESPIRATION_AND_RESPIRATORY_ELECTRON_TRANSPORT",
        "code": "R",
        "label": "Respiration / electron transport",
        "color": "#0072B2",
        "expected_members": {"COA3", "CYC1", "DMAC2", "NDUFB11", "SLC25A4"},
    },
    {
        "pathway": "REACTOME_MITOCHONDRIAL_TRANSLATION_ELONGATION",
        "code": "T",
        "label": "Mitochondrial translation elongation",
        "color": "#D55E00",
        "expected_members": {"MRPL34", "MRPL4", "MRPS34"},
    },
    {
        "pathway": "WP_SELENIUM_METABOLISM_AND_SELENOPROTEINS",
        "code": "S",
        "label": "Selenium metabolism / selenoproteins",
        "color": "#009E73",
        "expected_members": {"SELENOM", "SELENOW"},
    },
)
...
def configure_common() -> None:
    """Point shared pure helpers at the SELENOW analysis inputs."""
...
def load_support_rows() -> pd.DataFrame:
    ...
    require(not truth_series(rows["self_excluded"]).any(), "SELENOW was unexpectedly self-excluded")
    return rows
...
def load_graph() -> nx.DiGraph:
    ...
    require(nx.is_directed_acyclic_graph(graph), "The excitatory Bayesian network is not acyclic")
    require(graph.number_of_nodes() == 10441 and graph.number_of_edges() == 13759, "Unexpected network dimensions")
    require(set(graph.predecessors(DRIVER)) == EXPECTED_DIRECT_IN, "SELENOW direct incoming edges changed")
    require(set(graph.successors(DRIVER)) == EXPECTED_DIRECT_OUT, "SELENOW direct outgoing edges changed")
    return graph
...
def build_consensus_tables(
    ...
    for row in support_rows.itertuples(index=False):
        ...
        require(DRIVER in induced, f"SELENOW is absent from the effective background for {run_id}")
    ...
    require(neighborhood_occurrence[DRIVER] == SUPPORTING_RUNS, "SELENOW is not in all supporting neighborhoods")
    ...
    require(upstream_context == EXPECTED_UPSTREAM_CONTEXT, "SELENOW upstream context changed")

    display = display_for_hits(graph, retained_hits)
    display_nodes = set(display)
    require(display.number_of_nodes() == 25 and display.number_of_edges() == 25, "Unexpected display dimensions")
        ...
        node_rows.append(
            {
                ...
                "directed_depth_from_selenow": downstream_lengths.get(gene) if gene != DRIVER else 0,
                "upstream_depth_to_selenow": upstream_lengths.get(gene) if gene != DRIVER else None,
    ...
    edge_rows = [
        {
            ...
            "source": source,
            "target": target,
            "is_directly_incident_to_selenow": source == DRIVER or target == DRIVER,
...
def build_pathway_tables(graph: nx.DiGraph, nodes: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    require(len(background) == 6952 and len(displayed) == 23, "Unexpected ORA background or mapped display size")
        ...
        rows.append(
            {
                ...
                "background_definition": "all excitatory-neuron Bayesian-network genes represented in MSigDB C2:CP",
    ...
    require(len(rows) == 1739, f"Unexpected eligible pathway-test count: {len(rows)}")
    ...
    for row, q_value in zip(rows, adjusted):
        ...
        row["selection_rule"] = "contextual nonredundant representative; no term passed BH FDR < 0.05" if theme else None
    ...
    for theme in PATHWAY_THEMES:
        ...
        require(float(result["bh_fdr"]) >= 0.05, f"Contextual pathway became FDR-significant: {theme['pathway']}")
    ...
    require(len(memberships) == 10, f"Expected 10 pathway membership rows, found {len(memberships)}")
...
def radial_positions(nodes: pd.DataFrame) -> tuple[dict[str, tuple[float, float]], float, str]:
    positions: dict[str, tuple[float, float]] = {
        DRIVER: (0.0, 0.0),
        "ATP5MPL": (-U1_RADIUS, 0.0),
        "LAMTOR5": (-U2_RADIUS, 0.0),
    }
...
def prepare_cytoscape_tables(
    ...
    cy_edges["direct_edge_bool"] = truth_series(cy_edges["is_directly_incident_to_selenow"])

    def display_label(row: pd.Series) -> str:
        upstream = pd.to_numeric(pd.Series([row["upstream_depth_to_selenow"]]), errors="coerce").iloc[0]
    ...
    cy_edges["direct_edge_class"] = cy_edges["direct_edge_bool"].map({True: "direct_selenow", False: "other_displayed"})
...
def add_annotations(network_suid: int) -> None:
    ...
    add_text("SELENOW-centered excitatory-neuron consensus network", title_x, title_y, "figure_title", 29, "bold", "#111111")
    add_text(
    ...
    add_text("Node size and x/14 show neighborhood recurrence", legend_x, legend_top + 52, "legend_recurrence_1")
    add_text("across the 14 supporting runs", legend_x, legend_top + 86, "legend_recurrence_2")
    add_text("U1/U2: upstream distance to SELENOW", legend_x, legend_top + 126, "legend_upstream")
    add_text("Thick black border: mitochondrial query hit at >=4/14", legend_x, legend_top + 166, "legend_query_hit")
    ...
    add_text("Contextual representatives; none passes BH FDR < 0.05", legend_x, pathway_heading_y + 39, "legend_pathway_note", 15, color="#555555")
    ...
    add_text("25 nodes / 25 edges", legend_x, scope_heading_y + 43, "legend_scope_counts")
    add_text("All SELENOW edges: 1 incoming, 7 outgoing", legend_x, scope_heading_y + 80, "legend_scope_direct")
    add_text("D3 paths to 13 recurrent query hits", legend_x, scope_heading_y + 117, "legend_scope_hits")
    add_text("All model edges among displayed nodes are retained", legend_x, scope_heading_y + 154, "legend_scope_edges")
    add_text("Display threshold >=4/14 is a coverage choice, not a significance cutoff", legend_x, scope_heading_y + 202, "legend_scope_threshold", 15, color="#555555")
...
def render_cytoscape(nodes: pd.DataFrame, edges: pd.DataFrame, membership: pd.DataFrame, output_dir: Path) -> None:
    ...
    network_suid = p4c.create_network_from_data_frames(
        nodes=cy_nodes,
        edges=cy_edges,
        title="Phase 18 SELENOW radial excitatory-neuron consensus network",
        collection="Phase 18 SELENOW deep dive",
        node_id_list="gene",
    ...
    export_log: dict[str, Any] = {
        ...
        "direct_outgoing_edges": 7,
        "supporting_run_count": SUPPORTING_RUNS,
        "query_hit_threshold": QUERY_HIT_THRESHOLD,
        "query_hit_count": len(EXPECTED_QUERY_HITS),
        "threshold_rationale": "4/14 retains 13 of 14 observed hits in a readable 25-node graph; 5/14 retains 10 hits and 21 nodes",
...
def write_graphml(path: Path, nodes: pd.DataFrame, edges: pd.DataFrame, membership: pd.DataFrame) -> None:
    graph = nx.DiGraph(network=NETWORK_NAME, driver=DRIVER, query_hit_display_threshold="4/14")
    codes_by_gene = {
...
def write_documentation(output_dir: Path, pathway_ora: pd.DataFrame) -> None:
    ...
    for theme in PATHWAY_THEMES:
        ...
        summaries.append(f"{theme['label']} ({int(row['overlap_gene_count'])} genes; BH FDR = {float(row['bh_fdr']):.3g})")
    caption = f"""# SELENOW excitatory-neuron consensus network with pathway outlines
...
Pathway ORA used 23 displayed genes represented in MSigDB C2:CP v2026.1. The explicit universe was 6,952 excitatory-neuron Bayesian-network genes represented in that collection. One-sided hypergeometric tests were run for 1,739 pathways with 15-500 mapped background genes and corrected by Benjamini-Hochberg. The three displayed pathways are contextual nonredundant representatives; none passed BH FDR < 0.05: {', '.join(summaries)}.
...
def validate_outputs(
    ...
    threshold_five = sensitivity.loc[sensitivity["threshold_count"].eq(5)].iloc[0]
    checks = [
        ("supporting_run_count", SUPPORTING_RUNS == 14, str(SUPPORTING_RUNS)),
        ("selected_threshold", QUERY_HIT_THRESHOLD == 4, "4/14"),
        ("threshold_5_hit_count", int(threshold_five["retained_query_hit_count"]) == 10, str(threshold_five["retained_query_hit_count"])),
        ("node_count", len(nodes) == 25, str(len(nodes))),
        ("edge_count", len(edges) == 25, str(len(edges))),
        ("query_hit_count", int(truth_series(nodes["is_query_hit_meeting_display_threshold"]).sum()) == 13, "13"),
        ("all_direct_incoming", int(truth_series(nodes["is_direct_incoming_neighbor"]).sum()) == 1, "1"),
        ("all_direct_outgoing", int(truth_series(nodes["is_direct_outgoing_neighbor"]).sum()) == 7, "7"),
        ("pathway_representative_count", len(selected) == 3, str(len(selected))),
        ("pathway_membership_rows", len(membership) == 10, str(len(membership))),
        ("no_fdr_significant_pathway", not (pd.to_numeric(pathway_ora["bh_fdr"]) < 0.05).any(), str((pd.to_numeric(pathway_ora["bh_fdr"]) < 0.05).sum())),
...
def run(output_dir: Path) -> None:
    ...
    write_graphml(output_dir / "phase18_selenow_excitatory.graphml", nodes, edges, pathway_membership)
    ...
    print("Display: 25 nodes, 25 edges, 13 query hits at >=4/14")
