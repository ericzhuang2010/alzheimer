...
usage <- function() {
  cat(
    ...
    "phase_18_key_driver_selection/visualize_phase18_sex_apoe_non_mt.R ",
...
plot_path <- file.path(data_dir, "phase18_sex_apoe_non_mt_plot_data.tsv")
row_path <- file.path(data_dir, "phase18_sex_apoe_non_mt_row_annotations.tsv")
...
draw_figure <- function() {
  ...
  draw_legend_panel()
  graphics::mtext(
    "Sex- and APOE-stratified support for non-MT key drivers",
  ...
  graphics::mtext(
    "Non-MT denotes genes outside core MitoCarta; network key-driver evidence prioritizes candidates but does not prove causality.",
    side = 1, outer = TRUE, line = 0.58, adj = 0.015,
    cex = 0.70, col = text_mid
  )
}

basename_value <- "phase18_sex_apoe_non_mt"
...
message(
  "Rendered non-MT sex/APOE dot heatmap: ", nrow(row_data),
