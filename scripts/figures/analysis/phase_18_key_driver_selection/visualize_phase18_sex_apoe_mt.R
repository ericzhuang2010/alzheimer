...
usage <- function() {
  cat(
    ...
    "phase_18_key_driver_selection/visualize_phase18_sex_apoe_mt.R ",
...
plot_path <- file.path(data_dir, "phase18_sex_apoe_mt_plot_data.tsv")
row_path <- file.path(data_dir, "phase18_sex_apoe_mt_row_annotations.tsv")
...
assert_true(nrow(row_data) == 30L, "Row annotations must contain 30 contexts")
assert_true(nrow(plot_data) == 360L, "Plotted data must contain 360 cells")
assert_true(
  all(row_data$case_id == "mt_driver") &&
    all(plot_data$case_id == "mt_driver"),
  "Unexpected case ID"
)
assert_true(
  identical(sort(unique(row_data$context_display_order)), seq_len(30L)),
  "Context display order must be 1 through 30"
)
assert_true(
...
assert_true(sum(row_data$circle_displayed) == 26L, "Circle-display flags do not sum to 26")
assert_true(
  all(
...
row_count <- nrow(row_data)
row_y <- function(context_order) row_count + 1 - context_order

draw_main_panel <- function() {
  graphics::par(mar = c(4.1, 0.4, 0.5, 0.2), xpd = FALSE)
  graphics::plot.new()
  graphics::plot.window(
    xlim = c(-6.75, 22.25), ylim = c(-0.70, row_count + 3.60),
    xaxs = "i", yaxs = "i"
  )
  ...
  graphics::segments(
    6.50, 0.45, 6.50, row_count + 0.45,
    col = "#909090", lty = 2, lwd = 0.9
  )

  graphics::par(xpd = NA)
  apoe_labels <- rep(c("APOE ε2", "APOE ε3/ε3", "APOE ε4"), 4L)
  graphics::text(
    seq_len(12L), rep(row_count + 1.10, 12L), apoe_labels,
    srt = 38, adj = c(0, 0.5), cex = 0.66, col = text_dark
  )
  sex_centers <- c(2, 5, 8, 11)
  graphics::text(
    sex_centers, rep(row_count + 2.05, 4L),
    c("Female", "Male", "Female", "Male"),
    cex = 0.76, font = 2, col = text_dark
  )
  graphics::segments(
    c(0.60, 3.60, 6.60, 9.60), row_count + 1.72,
    c(3.40, 6.40, 9.40, 12.40), row_count + 1.72,
    col = "#777777", lwd = 0.65
  )
  graphics::text(
    c(3.50, 9.50), rep(row_count + 3.05, 2L),
    c("AD-up mitochondrial query", "AD-down mitochondrial query"),
    cex = 0.88, font = 2, col = text_dark
  )
  graphics::segments(
    c(0.55, 6.55), row_count + 2.67,
    c(6.45, 12.45), row_count + 2.67,
    col = "#555555", lwd = 0.8
  )

  graphics::text(-6.55, row_count + 1.15, "Net.", cex = 0.67, font = 2, srt = 38, adj = c(0, 0.5))
  graphics::text(-6.10, row_count + 1.15, "Circle", cex = 0.67, font = 2, srt = 38, adj = c(0, 0.5))
  graphics::text(-5.72, row_count + 1.15, "Extended", cex = 0.67, font = 2, srt = 38, adj = c(0, 0.5))
  graphics::text(-5.50, row_count + 2.05, "Gene", cex = 0.72, font = 2, adj = c(0, 0.5))
  graphics::text(-3.12, row_count + 2.05, "Broad network", cex = 0.72, font = 2, adj = c(0, 0.5))
  graphics::text(mean(c(q_start, q_end)), row_count + 2.45, "Network aggregate q", cex = 0.72, font = 2)
  graphics::text(mean(c(q_start, q_end)), row_count + 1.85, "−log10(q), separate scale", cex = 0.67, col = text_mid)
  graphics::text(support_x, row_count + 2.20, "Support /", cex = 0.69, font = 2)
  graphics::text(support_x, row_count + 1.65, "usable", cex = 0.69, font = 2)
  graphics::text(mean(c(coverage_start, coverage_end)), row_count + 2.20, "Coverage", cex = 0.69, font = 2)
  graphics::text(coverage_label_x, row_count + 1.65, "usable / eligible", cex = 0.65, font = 2)
  graphics::text(mean(c(tier_x1, tier_x2)), row_count + 2.05, "Tier", cex = 0.68, font = 2, srt = 38)
  graphics::text(rank_x, row_count + 2.05, "Rank", cex = 0.68, font = 2, srt = 38)
...
draw_legend_panel <- function() {
  ...
  graphics::text(0.13, 0.335, "circle-displayed", adj = c(0, 0.5), cex = 0.67)
  graphics::points(0.07, 0.300, pch = 1, cex = 0.62)
  graphics::text(0.13, 0.300, "passing below display cap", adj = c(0, 0.5), cex = 0.67)
  graphics::points(0.07, 0.265, pch = 23, bg = "white", cex = 0.65)
  graphics::text(0.13, 0.265, "broader mito reference", adj = c(0, 0.5), cex = 0.67)
...
draw_figure <- function() {
  ...
  draw_legend_panel()
  graphics::mtext(
    "Sex- and APOE-stratified support for MT key drivers",
  ...
  graphics::mtext(
    "10 circle genes • 26 circle-displayed contexts • 30 total passing contexts • 360 cells",
    side = 3, outer = TRUE, line = 0.52, adj = 0.015,
    cex = 0.76, col = text_mid
  )
  graphics::mtext(
    "MT denotes genes inside the fixed 1,136-gene core MitoCarta inventory; network key-driver evidence prioritizes candidates but does not prove causality.",
    side = 1, outer = TRUE, line = 0.58, adj = 0.015,
    cex = 0.70, col = text_mid
  )
}

basename_value <- "phase18_sex_apoe_mt"
...
message(
  "Rendered MT sex/APOE dot heatmap: ", nrow(row_data),
