#!/usr/bin/env python3
"""Build and render the Phase 18 LAMTOR5 excitatory consensus network in Cytoscape."""
...
SUPPORTING_RUNS = 12
QUERY_HIT_THRESHOLD = 3
MAX_DOWNSTREAM_DEPTH = 3
MAX_UPSTREAM_DEPTH = 2
SCHEMA = "phase18_lamtor5_excitatory_consensus_v1"
OUTPUT_PREFIX = "phase18_lamtor5_excitatory_consensus"
FIGURE_STEM = "phase18_lamtor5_excitatory_consensus_network_pathways"
STYLE_NAME = "Phase18 LAMTOR5 excitatory radial consensus pathway outlines"

EXPECTED_DIRECT_IN = {"UQCR10"}
EXPECTED_DIRECT_OUT = {
    "ATP5IF1", "ATP5MPL", "ELOC", "MAGEH1", "MRPS36", "NDUFA6",
    "POP7", "TMEM11", "UQCRHL",
}
EXPECTED_UPSTREAM_CONTEXT = {"UQCR10"}
EXPECTED_QUERY_HITS = {
    "ATP5IF1", "ATP5MC2", "CHCHD10", "MRPL4", "NDUFA6", "NDUFB6",
    "TMEM11", "TMEM126A",
}
PATHWAY_THEMES = (
    {
        ...
        "color": "#0072B2",
        "expected_members": {"ATP5IF1", "ATP5MC2", "NDUFA6", "NDUFB6", "UQCR10"},
    ...
    },
    {
        ...
        "color": "#009E73",
        "expected_members": {"NDUFA6", "NDUFB6", "TMEM126A"},
        "must_be_significant": True,
    },
    {
        ...
        "color": "#D55E00",
        "expected_members": {"ATP5MC2", "TMEM11"},
...
def build_consensus_tables(
    ...
    require(display.number_of_nodes() == 17 and display.number_of_edges() == 16, "Unexpected display dimensions")
...
def build_pathway_tables(graph: nx.DiGraph, nodes: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    require(len(background) == 6952 and len(displayed) == 15, "Unexpected ORA background or mapped display size")
    ...
    require(len(memberships) == 11, f"Expected 11 pathway membership rows, found {len(memberships)}")
...
def add_annotations(network_suid: int) -> None:
    ...
    add_text("LAMTOR5-centered excitatory-neuron consensus network", title_x, title_y, "figure_title", 29, "bold", "#111111")
    add_text(
        "U1 is upstream context; D1-D3 nodes lie downstream; arrows follow the Bayesian-network direction",
    ...
    add_text("Node size and x/12 show neighborhood recurrence", legend_x, legend_top + 52, "legend_recurrence_1")
    add_text("across the 12 supporting runs", legend_x, legend_top + 86, "legend_recurrence_2")
    add_text("U1: upstream distance to LAMTOR5", legend_x, legend_top + 126, "legend_upstream")
    add_text("Thick black border: mitochondrial query hit at >=3/12", legend_x, legend_top + 166, "legend_query_hit")
    ...
    add_text("OXPHOS and complex I pass BH FDR < 0.05; cristae and mTORC1 are contextual", legend_x, pathway_heading_y + 39, "legend_pathway_note", 15, color="#555555")
    ...
    add_text("U1: upstream context; D1-D3: minimum downstream steps", legend_x, radial_heading_y + 43, "legend_radial_text")
    scope_heading_y = radial_heading_y + 110
    add_text("DISPLAY SCOPE", legend_x, scope_heading_y, "legend_scope_heading", 17, "bold", "#111111")
    add_text("17 nodes / 16 edges", legend_x, scope_heading_y + 43, "legend_scope_counts")
    add_text("All LAMTOR5 edges: 1 incoming, 9 outgoing", legend_x, scope_heading_y + 80, "legend_scope_direct")
    add_text("D3 paths to all 8 observed query hits", legend_x, scope_heading_y + 117, "legend_scope_hits")
    add_text("All model edges among displayed nodes are retained", legend_x, scope_heading_y + 154, "legend_scope_edges")
    add_text("Display threshold >=3/12 is a coverage choice, not a significance cutoff", legend_x, scope_heading_y + 202, "legend_scope_threshold", 15, color="#555555")
...
def render_cytoscape(nodes: pd.DataFrame, edges: pd.DataFrame, membership: pd.DataFrame, output_dir: Path) -> None:
    ...
    network_suid = p4c.create_network_from_data_frames(
        nodes=cy_nodes,
        edges=cy_edges,
        title="Phase 18 LAMTOR5 radial excitatory-neuron consensus network",
    ...
    export_log: dict[str, Any] = {
        ...
        "threshold_rationale": "3/12 retains all 8 observed hits in a readable 17-node graph; 4/12 loses TMEM126A and yields 16 nodes",
...
def write_graphml(path: Path, nodes: pd.DataFrame, edges: pd.DataFrame, membership: pd.DataFrame) -> None:
    graph = nx.DiGraph(network=NETWORK_NAME, driver=DRIVER, query_hit_display_threshold="3/12")
    codes_by_gene = {
...
def write_documentation(output_dir: Path, pathway_ora: pd.DataFrame) -> None:
    ...
    caption = f"""# LAMTOR5 excitatory-neuron consensus network with pathway outlines
...
def validate_outputs(
    ...
    threshold_four = sensitivity.loc[sensitivity["threshold_count"].eq(4)].iloc[0]
    ...
    checks = [
        ("supporting_run_count", SUPPORTING_RUNS == 12, str(SUPPORTING_RUNS)),
        ("selected_threshold", QUERY_HIT_THRESHOLD == 3, "3/12"),
        ("threshold_4_hit_count", int(threshold_four["retained_query_hit_count"]) == 7, str(threshold_four["retained_query_hit_count"])),
        ("node_count", len(nodes) == 17, str(len(nodes))),
        ("edge_count", len(edges) == 16, str(len(edges))),
        ("query_hit_count", int(truth_series(nodes["is_query_hit_meeting_display_threshold"]).sum()) == 8, "8"),
        ...
        ("pathway_representative_count", len(selected) == 4, str(len(selected))),
        ("pathway_membership_rows", len(membership) == 11, str(len(membership))),
        ("selected_fdr_significant_pathways", selected_significant == 2, str(selected_significant)),
        ("all_fdr_significant_pathways", all_significant == 10, str(all_significant)),
...
def run(output_dir: Path) -> None:
    ...
    write_graphml(output_dir / "phase18_lamtor5_excitatory.graphml", nodes, edges, pathway_membership)
    ...
    print("Display: 17 nodes, 16 edges, 8 query hits at >=3/12")
